...
volatile static BYTE codeBuffer[SIZE_BUFFER] =
{
    ...
    1, 1,                       // 2 SET_COLOR 1
    2, 0, 0, 0, 63, 1, 10,      // 7 FILL_REGION 0, 0, 319, 10
    3                           // 1 END_SCENE
};


static int pc = 4 + 4 + 2 + 7 + 2 + 7 + 1;  ///< Program counter - PC указывает номер текущего байта для исполнения.
...
int main(void)
{
    ...
    PORTBbits.RB1 = 1;              // Устанавливаем признак того, что дисплей готов к приёму следующей команды

    pointer = pmp_data;

    PMP_RunBuffer();                // Сначала рисуем тестовую картинку
    ...
    /*
    Painter_SetColor(1);
    Painter_FillRegion(50, 50, 50, 50);
    Painter_EndScene();

    Painter_SetColor(0);
    Painter_FillRegion(60, 60, 30, 30);
    Painter_EndScene();
    */

    // И переходим к ожиданию и выполнению внешних команд
...
void PMP_FillRectX(void)
{
    ...
    for (; col < 15; col++)
    {
        ...
        Painter_SetColor(col);
        Painter_FillRegion(x, y, 15, 15);
    }
...
{
//    Painter_SetColor(0);
//    Painter_FillRegion(0, 0, 319, 239);
