...
void HAL_PCD_SetupStageCallback(PCD_HandleTypeDef *hpcd)
{
    ...
    {                                                               //
        ...
        {                                                           //
            if (prevLength != 0)                                    //
            {                                                       //
                gBF.connectToHost = 1;                              // GOVNOCODE “аким вот замысловатым образом определ€ем, что к нам подконнектилс€ хост (
            }                                                       //
            else                                                    //
            {                                                       //
                gBF.connectToHost = 0;                              //
                Settings_Save();                                    // ѕри отконнекчивании сохран€ем настройки // WARN ¬озможно, это не нужно делать
            }                                                       //
...
void HAL_PCD_DataInStageCallback(PCD_HandleTypeDef *hpcd, uint8_t epnum)
{
    ...
    // \todo WARN здесь оказываем€ после каждой передачи пакета. ћожно отловить момент, когда передача закончена.
}
