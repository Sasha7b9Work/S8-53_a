...
static void TestFunc(int type);
static int coordX = 0;
static int direction = 1;
static int recvComs = 0;
...
const StructCommand comArray[] =
{
    ...
    {PMP_DrawVPointLine,    6},          // 12 Не используется
...
void PMP_SetPalColorL(void)
{
    ...
    mSetPaletteColor(color.w, number);
    EnablePalette();
}
...
{
    /*
        Рисует вертикальную линию из точек
        1,2 - x
        3 - y
        4 - delta
        5 - numPoints
    */
    ...
    SHORT x = NextShort();
    SHORT y = NextByte();
    SHORT delta = NextByte();
    SHORT numPoints = NextByte();
...
//        return;
    ...
    Painter_SetColor(colorBackground);
    Painter_FillRegion(0, 0, 319, 239);
    Painter_SetColor(colorDraw);
    Painter_FillRegion(coordX, coordX, 45, 45);
    Painter_SetColor(colorBackground);
    UXCHAR text[10] = "123";
    text[0] = 0x31;
    text[1] = 0x32;
    text[2] = 0x33;
    Painter_SetFont(1);
    Painter_DrawText(coordX, coordX, &text[0]);
    Painter_SetColor(colorDraw);
    Painter_DrawText(coordX + 10, coordX + 10, text);
